...
namespace    // unnamed, anonymous namespace
{
  ...
  constexpr bool floating_point_is_equal( T const lhs,  U const rhs,  double const EPSILON = 1e-4 ) noexcept
  {
    ///////////////////////// TO-DO (1) //////////////////////////////
      ...
      ///         do this instead:
      ///           return a < b;
    return std::abs(lhs - rhs) < EPSILON; //we must return a boolean to see if the difference of lhs and rhs is less than or within epsilon of each other, to find the difference me must subtract them both and find the absolute value of it
    /////////////////////// END-TO-DO (1) ////////////////////////////
  }
...
_price{price}{} //we must move the parameters into the attributes
/////////////////////// END-TO-DO (2) ////////////////////////////
...
_price{other._price}{} //we use a copy constructor to  use the copy object parameter and use the accesors
/////////////////////// END-TO-DO (3) ////////////////////////////
...
_price{other._price}{} //we move the object parametr using the accesor to get each of its attributes into the constructor
...
///////////////////////// TO-DO (5) //////////////////////////////
{
  ...
  if(this != &rhs) //we check to see if rhs exists and if it does exist we don't do it
  {
   

    _upcCode = rhs._upcCode; //we copy the parameter to the variable 
    _brandName =  rhs._brandName;//we copy the parameter to the variable 
    _productName = rhs._productName;//we copy the parameter to the variable 
    _price = rhs._price;//we copy the parameter to the variable 

    
  }
  return *this;//we retern a dereferenced object
}
/////////////////////// END-TO-DO (5) ////////////////////////////
...
{
  ///////////////////////// TO-DO (8) //////////////////////////////
  return _upcCode; //we return an upcCode string attribute
  /////////////////////// END-TO-DO (8) ////////////////////////////
}
...
{
  ///////////////////////// TO-DO (9) //////////////////////////////
  return _brandName;//we return an brandName string attribute
...
{
  ///////////////////////// TO-DO (10) //////////////////////////////
  return _productName;//we return an productName string attribute
...
{
  ///////////////////////// TO-DO (11) //////////////////////////////
  return _price;//we return an _price double attribute
...
{
  ///////////////////////// TO-DO (13) //////////////////////////////
  return std::move(_brandName); //we return the brand name attribute
...
{
  ///////////////////////// TO-DO (14) //////////////////////////////
  return std::move(_productName);//we return the product name attribute
...
GroceryItem & GroceryItem::upcCode( std::string newUpcCode ) &
{
  ...
  _upcCode = std::move(newUpcCode); //we move the newUpcode into _upcCode
  return *this; //we return the dereferecned object
  /////////////////////// END-TO-DO (15) ////////////////////////////
}
...
{
  ///////////////////////// TO-DO (17) //////////////////////////////
  _productName = std::move(newProductName);//we move the newProductName into _productName
  return *this;//we return the dereferecned object
...
std::weak_ordering GroceryItem::operator<=>( const GroceryItem & rhs ) const noexcept
{
  ...
  ///////////////////////// TO-DO (19) //////////////////////////////
  if(auto result = _upcCode <=> rhs._upcCode;result != 0)
  {
    return result;
  }
  if( auto result = _productName <=> rhs._productName;result != 0)
  {
    return result;

  }
  if( auto result = _brandName <=> rhs._brandName;result != 0)
  {
    return result;
  }
...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ...
  if(stream >> std::ws >> std::quoted(t._upcCode)     && stream >> std::ws >> com && com == ',' && 
     stream >> std::ws >> std::quoted(t._brandName)   && stream >> std::ws >> com && com == ',' && 
     stream >> std::ws >> std::quoted(t._productName) && stream >> std::ws >> com && com == ',' && 
     stream >> std::ws >> t._price)
  {
...
std::ostream & operator<<( std::ostream & stream, const GroceryItem & groceryItem )
{
  ...
  const std::string com = ",  ";
    stream << std::quoted(groceryItem.upcCode()) << com 
           << std::quoted(groceryItem.brandName()) <<  com 
           << std::quoted(groceryItem.productName()) << com 
           << groceryItem.price();
    return stream;
