...
std::size_t GroceryList::size() const
{
  ...
  return _gList_vector.size(); //we return the size of any of the containers where in this case we will return the size of the
  /////////////////////// END-TO-DO (1) ////////////////////////////
}
...
std::size_t GroceryList::find( const GroceryItem & groceryItem ) const
{
  ...
  {
    if(_gList_vector.at(i) == groceryItem) return i; //if it is found in the list we return the position of the place its located
  }
  return _gList_vector.size(); //if it doesnt exist we return the size of the list
  /////////////////////// END-TO-DO (2) ////////////////////////////
}
...
void GroceryList::insert( const GroceryItem & groceryItem, std::size_t offsetFromTop )        // insert provided grocery item at offsetFromTop, which places it before the current grocery item at offsetFromTop
{
  ...
  ///////////////////////// TO-DO (3) //////////////////////////////
   ...
   if(find(groceryItem)  != _gList_vector.size() ) //we check to see if its already in the list by checking to see if the find function doesn't the size meaning that its not in the list
   {
      return;
   }
  /////////////////////// END-TO-DO (3) ////////////////////////////
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ///////////////////////// TO-DO (4) //////////////////////////////
      ...
      if( _gList_array.size() ==  _gList_array_size) //if both sizes are equal then we throw an exception
      {
            //One of the major differences between extendable and fixed capacity vectors is an extendable vector's ability to add and fixed capacity can't add
            throw CapacityExceeded_Ex("You excededed the capacity" exception_location);
      }

      for(auto i = _gList_array.size() - 1; i !=offsetFromTop ;--i)
      {
...
_gList_vector.insert(std::next(_gList_vector.begin(), offsetFromTop), groceryItem);//we use the next function to convert the index to an iterator to add the item we only use insert
  ...
  { /**********  Part 3 - Insert into doubly linked list  **********/
   ...
   _gList_dll.insert(std::next(_gList_dll.begin(), offsetFromTop),groceryItem);//we use the next function to convert the index to an iterator to add the item we only use insert
  ...
  { /**********  Part 4 - Insert into singly linked list  **********/
    ///////////////////////// TO-DO (7) //////////////////////////////
      ...
      //segmentation fault
 _gList_sll.insert_after(std::next(_gList_sll.before_begin(), offsetFromTop), groceryItem); //we use the next function to convert the index to an iterator to add the item we only use isert after since its a singly linked list and .before_begin since its a signly linked list
...
for(std::size_t i = offsetFromTop + 1 ; i != _gList_array_size;++i)
  {
  ...
  --_gList_array_size; //we have the adjust the _gList_array_size or else an error will occur
_gList_array.at(_gList_array_size) = GroceryItem();
 ...
 _gList_vector.erase(std::next(_gList_vector.begin(), offsetFromTop));//we use the next function to convert the index to an iterator to remove the item we only use erase
...
_gList_dll.erase(std::next(_gList_dll.begin(), offsetFromTop));//we use the next function to convert the index to an iterator to remove the item we only use erase
  ...
  _gList_sll.erase_after(std::next(_gList_sll.before_begin(), offsetFromTop)); //we use the next function to convert the index to an iterator to remove the item, we only use erase_after since its a singly linked list
...
void GroceryList::moveToTop( const GroceryItem & groceryItem )
{
  ...
  if(find(groceryItem) != _gList_array_size) //we check to see if already have the item in the list by checking to see if it doesnt return the size,if it returns the size it indicates that it doesnt exist
  {
       remove(groceryItem);//we remove it if it doesnt exist
      insert(groceryItem);//we reinsert it if it does exist
...
GroceryList & GroceryList::operator+=( const std::initializer_list<GroceryItem> & rhs )
{
 ...
 for(auto i = rhs.begin(); i != rhs.end();i+=1)//we use the += opertor to iterate over the initializer list and we use iterators to iterate over it
  {
      insert( *i, Position::BOTTOM ); //we insert everyting at the bottom of the list
 }
...
GroceryList & GroceryList::operator+=( const GroceryList & rhs )
{
 ...
 for(auto i : rhs._gList_vector) //we use the += opertor to iterate over the regular list
  {
      insert(i, Position::BOTTOM); //we insert everyting at the bottom of the list
...
std::weak_ordering GroceryList::operator<=>( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (15) //////////////////////////////
    ...
    //we use nested for loops to compare the two lists, the outer loop(_gList_vector) at each index compares all the indexes of the inner loop(rhs._gList_vector)
    for(GroceryItem item1 : _gList_vector) //we loop through the current container
      {
        for(GroceryItem  item2: rhs._gList_vector) // we loop through the container of rhs
        {
          auto result = item1 <=> item2; //we use a three way compariosn to compare the two items of each loop
          if(result != 0) //we check to see if the two items don't equal 0
          {
            return result; //we return the result of the comparison
          }
        }
      }
  

    return  _gList_vector.size() <=> rhs._gList_vector.size(); //if no differences were found,return the result of the comparison
...
bool GroceryList::operator==( GroceryList const & rhs ) const
{
  ...
  bool sizeIsEqual = _gList_vector.size() == rhs._gList_vector.size(); //we check to see if the size of the vector is equal to each other
    ...
    return sizeIsEqual; //we return the boolean value if could true on all other conditions or false if the size is different
...
std::istream & operator>>( std::istream & stream, GroceryList & groceryList )
{
  ...
  while(stream >> item) //we are going interate using a vector
  {
    
    groceryList.insert(item,GroceryList::Position::BOTTOM); //if it meets the criteria we insert it into the temprary list 
  }
